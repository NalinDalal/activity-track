...
export default function LoginPage() {
  ...
  return (
    <section className="flex overflow-hidden relative justify-center items-center py-16 px-4 min-h-screen text-white bg-[#ff6347]">
      {/* Tomato Background Image (bottom left, rotated) */}
      <img
        src="/tomato.jpg"
        alt="Tomato Background"
        className="absolute w-80 opacity-10 pointer-events-none select-none bottom-[-40px] left-[-40px] rotate-[-15deg]"
      />
      ...
      <div className="z-10 p-8 w-full max-w-md rounded-2xl border shadow-xl backdrop-blur-lg bg-white/10 border-white/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {error && <p className="text-center text-500">{error}</p>}
          <div>
